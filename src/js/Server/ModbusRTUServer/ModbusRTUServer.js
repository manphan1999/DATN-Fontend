...
const ListRTUServer = () => {
    const [actionRTUServer, setActionRTUServer] = useState([]);
    ...
    const fetchRTUServer = async () => {
        setLoading(true);
        let response = await fetchAllRTUServer();
    ...
    const handleOpenModalAdd = () => {
        setActionRTUServer('CREATE RTU');
        setOpenModalAdd(true);
    }
    const handleCloseModalAddServer = () => { setOpenModalAdd(false); fetchRTUServer(); }

    const handleCloseModalSearch = () => { setOpenModalSearchTag(false); fetchRTUServer(); }

    const handleEditRTUServer = (row) => {
        setDataModalServer(row);
        setActionRTUServer('UPDATE RTU');
        setOpenModalAdd(true);
    }

    const handleCloseModalDelete = () => { setIsShowModalDelete(false); fetchRTUServer(); }

    const handleDeleteRTUServer = (row) => {
        let dataToDelete = [];
        if (row) {
            dataToDelete = [{ id: row.id }];
            setSelectedCount(1);
        } else {
            dataToDelete = listRTUServer
                .filter(item => selectedRows.includes(item.id))
                .map(item => ({ id: item.id }));
            setSelectedCount(dataToDelete.length);
        }
        setDataModalDelete(dataToDelete);
        setActionDelete('RTUSERVER');
        setIsShowModalDelete(true);
    };

    const conformDeleteRTUServer = async () => {
        if (!dataModalDelete || dataModalDelete.length === 0) return;
        const res = await deleteRTUServer({ list: dataModalDelete });
        if (res && res.EC === 0) {
                ...
                <>
                    <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 1, height: '100%', }}  >
                        {/* <Button
                            ...
                            onClick={(e) => { e.stopPropagation(); handleEditRTUServer(params.row); }}
                        >
                            Sửa
                        </Button> */}
                        ...
                        <Button
                            ...
                            sx={{ textTransform: 'none', minWidth: 80 }}
                            onClick={(e) => { e.stopPropagation(); handleDeleteRTUServer(params.row); }}
                        >
                            Xóa
                        </Button>
                    </Box>
    ...
    return (
        <div>
            ...
            <ModalDelete
                ...
                action={actionDelete}
                conformDeleteRTUServer={conformDeleteRTUServer}
